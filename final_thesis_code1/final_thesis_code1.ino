...
// --- New Addition: Define the onboard LED pin ---
#define ONBOARD_LED 2  // ESP32 onboard LED is typically on pin 2
...
void setup() {
  ...
  // --- New Addition: Initialize the onboard LED pin ---
  pinMode(ONBOARD_LED, OUTPUT);
  digitalWrite(ONBOARD_LED, LOW); // Start with LED off
  Serial.println("ESP32: Onboard LED (Pin 2) initialized");
...
void loop() {
  ...
  // --- New Addition: Variables for LED blinking ---
  static unsigned long lastLedToggle = 0;
  static bool ledState = false;
  const unsigned long ledInterval = 1000; // 1 second interval for LED toggle

  unsigned long currentTime = millis();

  // --- New Addition: Toggle the onboard LED every 1 second ---
  if (currentTime - lastLedToggle >= ledInterval) {
    ledState = !ledState; // Toggle LED state
    digitalWrite(ONBOARD_LED, ledState ? HIGH : LOW);
    Serial.println("ESP32: Onboard LED " + String(ledState ? "ON" : "OFF"));
